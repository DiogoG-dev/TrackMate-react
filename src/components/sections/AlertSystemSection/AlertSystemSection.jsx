import './AlertSystemSection.css'

export function AlertSystemSection() {
    return (
        <section className="alert-system-section">
            <div className="alert-system-content">
                <h2><i className="fa-solid fa-exclamation"></i>SISTEMA DE ALERTA</h2>
                <h3>Deixe seu corpo te guiar</h3>
                <p>
                    Com o <b>TrackMate 0.4 XE</b>, você recebe alertas em tempo real baseados em seu ritmo e desempenho. Respeite seus limites, 
                    evite excessos e evolua com inteligência.
                </p>
            </div>
        </section>
    )
}